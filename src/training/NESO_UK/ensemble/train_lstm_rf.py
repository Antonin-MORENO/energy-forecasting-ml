...
# Load the retrained Random Forest model and the best LSTM checkpoint + its y-scaler
...
# Load Random Forest and LSTM models
rf   = joblib.load(rf_path)

# Prepare LSTM input geometry with the correct scaling
dh_lstm = DataHandler(
...
# Load the preprocessed sequential + static features for each dataset split
X_seq_tr, X_stat_tr, y_tr_seq, \
X_seq_val, X_stat_val, y_val_seq, \
X_seq_te,  X_stat_te,  y_test_seq = \
    dh_lstm.get_sequence_data(val_years=1)

# Initialize and load trained LSTM model
lstm_params = {
...
# Load data for Random Forest (standard scaling) 
# This ensures the RF model gets the input in the format it was trained on
dh_std = DataHandler(
...
# Generate validation predictions (no need to time these) 
pred_val_rf    = rf.predict(X_val_std)
...
# Find optimal weight 'w' for combining LSTM and RF using validation RMSE 
# Try different combinations from 0 (RF only) to 1 (LSTM only)
ws   = np.linspace(0, 1, 1001)
best = (0.0, np.inf)
for w in ws:
    comb = w * pred_val_lstm + (1 - w) * pred_val_rf
    rmse = np.sqrt(mean_squared_error(y_val_std, comb))
    if rmse < best[1]:
        best = (w, rmse)
w_lstm, val_rmse = best

# Generate test predictions and measure inference time 
...
# Ensemble
t0 = time.perf_counter()
pred_ens_test = w_lstm * pred_lstm_test + (1 - w_lstm) * pred_rf_test
time_comb = time.perf_counter() - t0

time_ens_total = time_rf + time_lstm + time_comb


# Compute evaluation metrics 
def calc_metrics(y, p, inf_time):
    err   = y - p
...
metrics = {
    'rf':   calc_metrics(y_test_std, pred_rf_test, time_rf),
...
print("=== Test metrics ===")
print("RF   standalone:", metrics['rf'])
print("LSTM standalone:", metrics['lstm'])
print("Ensemble      :", metrics['ens'])

# Save results 
# Store optimal weight, validation RMSE, and all test metrics
out = {
    'weight_lstm':    round(w_lstm,3),
    'weight_rf':      round(1-w_lstm,3),
    'val_rmse':       round(val_rmse,4),
    'test_metrics':   metrics
}
out_dir = 'outputs/evaluations/ensemble_lstm_rf'
os.makedirs(out_dir, exist_ok=True)
with open(os.path.join(out_dir, 'ensemble_results.json'), 'w') as f:
    json.dump(out, f, indent=2)

print("→ Ensemble LSTM+RF enregistré dans", out_dir)
