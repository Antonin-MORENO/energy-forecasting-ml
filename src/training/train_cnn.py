...
exp_name    = 'cnn_all_lags_experiment'
...
history  = cnn.fit(X_train_dict, y_tr_z, X_val=X_val_dict, y_val=y_val_z)
train_tm = time.perf_counter() - t0

# Hold-out evaluation (inverse transform predictions)
preds_z  = cnn.predict(X_test_dict)
